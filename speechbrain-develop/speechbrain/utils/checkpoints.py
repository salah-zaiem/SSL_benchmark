...
def torch_parameter_save(obj, path):
    """Saves the obj's parameters to path.
    Default save hook for torch.nn.Modules
    For saving torch.nn.Module state_dicts.
    Arguments
...
def torch_parameter_recovery(obj, path,end_of_epoch, device=None):
    """Loads a torch.nn.Module state_dict from the given path instantly.
    This can be made the default for torch.nn.Modules with:
    """
    obj.data = torch.load(path)
...
DEFAULT_LOAD_HOOKS = {
    ...
    torch.nn.Parameter: torch_parameter_recovery
}
DEFAULT_SAVE_HOOKS = {
    ...
    torch.nn.Parameter: torch_parameter_save
}
